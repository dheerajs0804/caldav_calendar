...
class rcube
{
	...
	###check imap connection status
        $imapconnectionstatus = $this->imap->check_connection();

        ###if imap connection success, then send mail, else give failed error.
        if($imapconnectionstatus){
                // send message
                $sent     = $this->smtp->send_mail($from, $a_recipients, $smtp_headers, $msg_body, $options);
                $response = $this->smtp->get_response();
                $error    = $this->smtp->get_error();
                ...
                self::console("failed to sent mail, due to imap connection failed");
                //$error = "Failed to send mail. Please refresh and try again.";
		$error = "There was a temporary error in sending the message. Please wait for a minute and save the message as a draft. Send the mail from the draft folder. ";
        }

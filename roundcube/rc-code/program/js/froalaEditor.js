...
function rcube_text_editor(config, id)
{
  ...
  function init_froala(selector){
	...
	spellcheck: true,
        ...
        toolbarButtonsMD: ['plaintext','|', 'bold', 'italic', 'fontFamily', 'fontSize', 'align', 'insertImage', 'insertFile' ,'undo', 'redo','html'],

        toolbarButtonsSM: ['plaintext','|', 'bold', 'italic', 'fontFamily', 'fontSize', 'align', 'insertImage', 'insertFile' ,'undo', 'redo','html'],

        toolbarButtonsXS: ['plaintext','|', 'bold', 'italic', 'underline','fontFamily', 'fontSize','align', 'undo', 'redo','html'],

        pluginsEnabled: ['align', 'colors','codeView', 'draggable','emoticons', 'entities', 'file', 'filesManager', 'fontAwesome', 'fontFamily', 'fontSize', 'fullscreen', 'image', 'imageManager', 'inlineStyle', 'inlineClass', 'lineBreaker', 'lineHeight', 'link', 'lists', 'paragraphFormat', 'paragraphStyle', 'quickInsert', 'quote', 'save','specialCharacters','spellChecker', 'table', 'url', 'video', 'wordPaste', 'codeBeautifier'],
	...
	events: {
		...
		const file_type = type.substring(0 ,type.indexOf('/'));

		//add base64 image for signature and allowed image size is 64KB
                if(config.mode == 'identity'){
                        var editor = this;
                        var reader = new FileReader();
			if(file_type != "image"){
       				runtime_error_for_nonimage("nonImage_identity");
			}
                        else if (images.length && images[0].size < 65536) {
                                reader.onload = function(e) {
                                        var result = e.target.result;
                                        result = result.replace("base64;base64", "base64");
                                        editor.image.insert(result, null, null, editor.image.get());
                                };
                                reader.readAsDataURL(images[0]);
                        }
                        else if(images.length && images[0].size > 65536 && !document.getElementById('image_upload').value){
                                reader.onload = function(e) {
                                        var result = '';
                                        editor.image.insert(result, null, null, editor.image.get());
                                };
		...
		//allow paste image using base64 for image size of 700KB
                if(!document.getElementById('image_upload').value && config.mode != 'identity'){
                   var editor = this;
                   var reader = new FileReader();
		   if(file_type != "image"){
      			runtime_error_for_nonimage("nonImage_compose");
		   }
                   else if (images.length && images[0].size < 716800) {
                        reader.onload = function(e) {
                                var result = e.target.result;
                                result = result.replace("base64;base64", "base64");
                                editor.image.insert(result, null, null, editor.image.get());
		...
		//generate runtime error for non-image copy paste event
   		function runtime_error_for_nonimage(type){
		...
		if(error.code == 1 && !document.getElementById('image_upload').value){
		    if(localStorage.getItem("file_type") == "nonImage_identity"){
                        localStorage.removeItem("file_type");
                        $layer.find('h3').text('Non-image files cannot be uploaded.');
                    }
    	            else if(localStorage.getItem("file_type") == "nonImage_compose"){
        	                localStorage.removeItem("file_type");
                	        $layer.find('h3').text('Attach a file other than an image by clicking on "Attach a file" or by using "Upload File" option.');
               	    }
                    else if(config.mode == 'identity')
                        $layer.find('h3').text('Image cannot be greater than 64 KB');
                    else
                        $layer.find('h3').text('Embedded image size cannot exceed 700 KB. For images above 700 KB use the inline uploader OR attach the image as a file.');
                }
  ...
  this.toggle = function(ishtml, noconvert)
  {
	...
	if(rcmail.env.action == 'compose'){
                ...
                document.getElementById('rcmfd_signature').style.display = 'none';
                document.getElementsByClassName('editor-toolbar')[0].style.display = 'none';
        }
	...
	if(rcmail.env.action == 'compose'){
                ...
                document.getElementById('rcmfd_signature').style.display = 'block';
                document.getElementsByClassName('editor-toolbar')[0].style.display = 'block';
        }

...
class rcmail extends rcube
{
    ...
    public function display_server_error($fallback = null, $fallback_args = null, $suffix = '', $params = array())
    {
		...
		self::console("imap connection stopped");
            }
        }
        else if ($err_code < 0) {
            $error = 'storageerror2';
	...
	###check if editor value is present in users session
	if ( $_SESSION['bayaeditor'] == "" ){
		/// call WS and get editor value
		$editor_prop = $this->domainWS();

		///set value into users session
		$_SESSION['bayaeditor'] = $editor_prop;
	}else{
		///get editors value from users session
		self::console("value from users session");
		$editor_prop = $_SESSION['bayaeditor'];
	}

        //$editor_prop = $this->domainWS();

        if($editor_prop == "Froala"){
            foreach ($lang_codes as $code) {
                if (file_exists("$assets_dir/program/js/froala/languages/$code.js")) {
                    $lang = $code;
                    break;
                }
            }
            ...
            $this->output->include_script('froala/froala_editor.pkgd.min.js');
            $this->output->include_script('froalaPlugins.js');
            $this->output->include_script('froalaEditor.js');
        }
        else{
            foreach ($lang_codes as $code) {
	...
	$loginuserpasswosd = $this->decrypt($_SESSION['password']);
	$loginhost = $this->config->get('default_host');
	$timeout = $this->config->get('ws_timeout');
	$connectiontimeout = $this->config->get('ws_connectiontimeout');

	###extract username and domain
	
	list($tmpname,$tmpdomain) = explode('@', $loginuser);
        $WS_uname = $tmpname;
        $WS_domain = $tmpdomain;

	self::console($WS_uname);
        self::console($WS_domain);
	self::console($loginhost);
	self::console($timeout);
	self::console($connectiontimeout);
 
	###get mailclientbayav4editor property from domain level
               
	//call WS and check which editor needs to be enable for domain into bayav4
        $url = 'http://'.$loginhost.':8080/orchestration.ws/domain/'.$WS_domain.'?properties=mailclientbayav4editor&absolutevalues=false';

        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_TIMEOUT, $timeout);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $connectiontimeout);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_USERPWD, "$loginuser:$loginuserpassword");
        ...
        $prop = $decodejson['result']['mailclientbayav4editor'];
        ...
        if ($_SESSION['storage_host'] && !$storage->is_connected()) {
            ...
            if (!$storage->connect($host, $user, $pass, $port, $ssl)) {
                if (is_object($this->output)) {
                    $this->output->show_message('storageerror2', 'error');
                }
            }
            else {
                $this->set_storage_prop();

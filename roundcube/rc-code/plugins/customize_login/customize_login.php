...
class customize_login extends rcube_plugin {
  ...
  function init() {
    ...
    $this->register_action("plugin.reset_background_to_default", array($this, "reset_background_to_default_handler"));
    $this->register_action("plugin.toggle_bg_reset_button", array($this, "toggle_bg_reset_button_handler"));
  ...
  function getMediaFileNames() {
    ...
    $this->bg_default = $this->rc->config->get('bg_default');
  }
  ...
  function create_default_frames_handler() {
    ...
    try {
       ...
       file_put_contents($this->targetFolder . $this->domainFolder . $this->bg_default, 'true');
  ...
  function toggle_bg_reset_button_handler() {
    try {
      $fileContent = file_get_contents($this->targetFolder . $this->domainFolder . $this->bg_default);
      $this->rcmail->output->command('plugin.toggleBackgroundResetButton', array('bgDefault' => $fileContent));
    } catch (Exception $e) {
      $this->rcmail->output->show_message($this->gettext('reset_toggle_failed'), 'error');
    }
  }

  function reset_background_to_default_handler() {
    $defaultStyle = 'background-image: url("skins/elastic/SkyconnectLoginPageOptimal.jpg"); background-repeat: no-repeat; background-size: cover;';
    try {
      file_put_contents($this->targetFolder . $this->domainFolder . $this->bg_choice, 'image');
      file_put_contents($this->targetFolder . $this->domainFolder . $this->bg_style, $defaultStyle);
      file_put_contents($this->targetFolder . $this->domainFolder . $this->bg_default, 'true');
      $this->rcmail->output->command('plugin.resetBackgroundDefault', array('isReset' => true));
    } catch (Exception $e) {
      $this->rcmail->output->show_message($this->gettext('reset_default_failed'), 'error');
    }
  }
  ...
  function populate_bg_img_preview_handler() {
    ...
    if (!is_null($img)) {
      ...
      $this->rcmail->output->command('plugin.populateBgImgPreview', array('imgFile' => $file, 'bgDefault' => $fileContent));
    } else {
      $this->rcmail->output->command('plugin.populateBgImgPreview', array('imgFile' => false, 'bgDefault' => $fileContent));
    }
  }
  ...
  function save_contents_as_media_files($content, $action) {
	...
	$retVal = file_put_contents($targetFolder . $this->bg_default, 'false');
        break;
      case 'bgcolor-save':

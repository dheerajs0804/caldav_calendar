...
class libvcalendar implements Iterator
{
    ...
    private $attendee_keymap = array('email' => 'EMAIL', 'name' => 'CN', 'status' => 'PARTSTAT', 'role' => 'ROLE',
  ...
  // Indicates debug mode for CalDAV
    ...
    */
    public function import($vcal, $charset = 'UTF-8', $forward_exceptions = false, $memcheck = true)
    {
		...
		//self::debug_log( "lvcal: import: ".json_encode($vcal ) );
	$currenttime=time();

  	$timestramp=date("Y_m_d_h_i_s",$currenttime);
  	$copyicsfile="/tmp/copyics_" .$timestramp;
	$newicsfilename="/tmp/newics_" .$timestramp;

  	file_put_contents($copyicsfile, $vcal);

  	shell_exec("sh /var/www/html/roundcubemail/plugins/calendar/rewrite_ics_file.sh $copyicsfile $newicsfilename");

	$vcal=file_get_contents($newicsfilename);

            $vobject = VObject\Reader::read($vcal, VObject\Reader::OPTION_FORGIVING | VObject\Reader::OPTION_IGNORE_INVALID_LINES);
            if ($vobject)
		{
		self::debug_log( "lvcal: import: vobject created: ".json_encode($vobject ) );
    ...
    */
    ...
    public function import_from_vobject($vobject)
    {
		...
		self::debug_log( "lvcal: import_from_vobject: started" );
        if ($vobject->name == 'VCALENDAR') {
		self::debug_log( "lvcal: import_from_vobject: VCALENDAR" );
			...
			self::debug_log( "lvcal: import_from_vobject: ve: ".json_encode($ve) );
		...
		self::debug_log( "lvcal: import: vobject returning: ".json_encode($this->objects ) );
        return $this->objects;
    }
    ...
    {
	 self::debug_log( "in to_array".json_encode( $ve ) );
        ...
        foreach ($ve->children as $prop) {
            if (!($prop instanceof VObject\Property))
                continue;
	    self::debug_log( "foreach (ve->children as prop) ".$prop->name  );
            switch ($prop->name) {
            case 'DTSTART':
            case 'DTEND':
            case 'DUE':
            ...
            case 'ORGANIZER':
                ...
                self::debug_log( "in to_array".json_encode( $params ) );
                $attendee = self::map_keys($params, array_flip($this->attendee_keymap));
                self::debug_log( "in to_array before replace mailto".json_encode( $attendee ) );
                // iOS device do add attendee status, set default as NEEDS-ACTION
                if(empty($attendee['status'])) {
                    $attendee['status'] = 'NEEDS-ACTION';
                }
                // for ccs server $prop->value is urn:x-uid:user_06_11_2009_16_13_12 for local users
                // This cause incompatibility when the data is posted back to the caldav server
                // we get attendee email from the EMAIL param of the attribute
                // ORIGINAL CODE: $attendee['email'] = preg_replace('/^mailto:/i', '', $prop->value);
                if( empty ( $params["EMAIL"] ) )
                    $attendee['email'] = preg_replace('/^mailto:/i', '', $prop->value);
                else
                    $attendee['email'] = preg_replace('/^mailto:/i', '', $params["EMAIL"] );
                self::debug_log( "in to_array after replace mailto".json_encode(  $attendee ) );
	...
	self::debug_log( "before unset".json_encode( $event ) );
		...
		self::debug_log( "unset start: instanceof ".get_class($event['start']));
            unset($event['start']);
        }
		...
		self::debug_log( "unset end: instanceof ".get_class($event['end']));
            unset($event['end']);
        }
	self::debug_log( json_encode( $event ) );
        // minimal validation
...
//        }
    ...
    private function _to_ical($event, $vcal, $get_attachment, $recurrence_id = null)
    {
	  ...
	  $ve->add('SEQUENCE', 0);
        ...
        foreach ((array)$event['attendees'] as $att) {
            self::debug_log( "in attendee : ".json_encode($att ) );
            if (is_array($att)) {
                self::debug_log( "is array attendee : ".json_encode($att ) );
                $attendee = $att;
                if ($attendee['role'] == 'ORGANIZER') {
                    if (empty($event['organizer']))
                        $event['organizer'] = $attendee;
                } else {
                    self::debug_log( " att : ".json_encode($attendee ) );
                    if (!empty($attendee['email'])) {
                        if (isset($attendee['rsvp'])) {
                            $attendee['rsvp'] = $attendee['rsvp'] ? 'TRUE' : null;
                        }
                        $ve->add('ATTENDEE', 'mailto:' . $attendee['email'], self::map_keys($attendee, array_filter( $this->attendee_keymap)));
                    }
                }

            } else {
                self::debug_log( "is NOT array attendee before decode : ".getType( $att ));
                $iattendee = json_decode( $att, true );
                self::debug_log( "is NOT array attendee : ".json_encode( $attendee ));
                foreach( $iattendee as $attendee ) {
                    if ($attendee['role'] == 'ORGANIZER') {
                        if (empty($event['organizer'])) {
                            $event['organizer'] = $attendee;
                        }
        ...
        if ($event['organizer']) {
            ...
            $ve->add('ATTENDEE', 'mailto:' . $event['organizer']['email'], self::map_keys($event['organizer'], array_filter( $this->attendee_keymap)));
        }
 ...
 */
class vobject_location_property extends VObject\Property\Compound
{
    /**

...
class caldav_client extends Sabre\DAV\Client
{
    ...
    /**
     * OAuth2 authentication for Sabre/DAV/Client
     */
    const AUTH_OAUTH = 4;
     ...
     * @param oauth_client $oauth_client Optional OAuth2 client instance to be used instead of HTTP Basic Auth.
     */
    public function __construct($uri, $user = null, $pass = null, oauth_client $oauth_client = null)
    {
        ...
        if($oauth_client) {
            $settings["authType"] = self::AUTH_OAUTH;
    ...
    private function handle_error(Sabre\Dav\Exception $err, $log = null, $terminate = null)
    {
        // If this error is caused by invalid oauth token, don't raise an error but show a
        // message to refresh the token
        if($this->oauth_client && $err->getHTTPCode() == 401) {
            $rc = rcmail::get_instance();
            $rc->output->show_message("calendar.oauth2.manual_refresh_notice", "notice",
                array("authorize_uri" => $this->oauth_client->get_authorize_uri()));
            return;
        }
    ...
    protected function curlRequest($url, $settings) {
        ...
        if($this->oauth_client) {
            try {
                ...
                array_push($settings[CURLOPT_HTTPHEADER],
                    sprintf('Authorization: Bearer %s', $access_token->getAccessToken()));
            }
            catch(oauth_exception $err)
            {
                // Silently ignore missing access_token, they will be handled in caldav_client::handle_error()
            }
        }
    ...
    public function get_ctag()
    {
        ...
        {
            $this->handle_error($err, true, false);
        }
    ...
    public function put_event($path, $event, $etag = null)
    {
        try
        {
            $headers = array('User-Agent' => 'Roundcube',"Content-Type" => "text/calendar; charset=utf-8");

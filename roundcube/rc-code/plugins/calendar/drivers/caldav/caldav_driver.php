	...
	*/
	  ...
	  public function __construct($cal)
	  {
	      ...
	      $this->attachments = $this->rc->config->get("caldav_attachment_support", "True");
	  ...
	  public function calendar_form($action, $calendar, $formfields)
	  {
		  ...
		  if (!empty($tab['fieldsets']) && is_array($tab['fieldsets'])) {
			  ...
			  if ($subcontent) {
			      $content .= html::tag('fieldset', null, html::tag('legend', null, MQ($fieldset['name'])) . $subcontent) ."\n";
			  }
		      }
		  }
		  else {
		  ...
		  if ($content) {
		      $this->form_html .= html::tag('fieldset', null, html::tag('legend', null, MQ($tab['name'])) . $content) ."\n";
		  }
	      }
	  ...
	  private function get_form_part($form)
	  {
		  ...
		  if(isset($form["style"]) && $form["style"] == "table")
		  {
			  ...
			  $table->add('title', html::label($colprop['id'], MQ($label)));
		  ...
		  else
		  {
			  ...
			  $content .=
				  ...
				  html::label( $colprop['id'], MQ( $label ) ) .
				  html::br() .
				  $colprop['value'] );
		      }
		  }
    ...
    private function parseFreeBusyInformation($fbdata) {
        ...
        if ($fbdata) {
            ...
            $ical->import($fbdata);
            if ($fb = $ical->freebusy) {
                self::debug_log("parseFreeBusyInformation: Imported iCal: FreeBusy ".print_r($fb, true));
                $result = array();
    ...
    private function getFreeBusyForEmail($email, $start, $end) {
        $result = array();

        try {
            if(!class_exists("MithiCalDavFreeBusyClient")) {
                require_once(dirname(__FILE__).'/../../../../vendor/mithi/caldav/client/MithiCalDavFreeBusyClient.php');
            }

            $caldavserver = $this->rc->config->get ('mithi_calendar_server');
            $calendaruser = $this->rc->user->data['username'];
            $calendaruserpassword = $this->rc->decrypt($_SESSION['password']);
            $mithiCalDavFreeBusyClient = new mithi\caldav\client\MithiCalDavFreeBusyClient($caldavserver, $calendaruser, $calendaruserpassword);

            // Get date time in 20180815T101930Z format
            $startDateTime = gmdate("Ymd\This\Z", $start);
            $endDateTime = gmdate("Ymd\This\Z", $end);

            // Get free/busy infromation from calendar server
            $freeBusyInfo = $mithiCalDavFreeBusyClient->getFreeBusyForEmail($email, $startDateTime, $endDateTime);
            self::debug_log("getFreeBusyForEmail: Mithi Calendar Server free/busy information ".print_r($freeBusyInfo, true));

            $result = $this->parseFreeBusyInformation($freeBusyInfo);
            self::debug_log("getFreeBusyForEmail: Parsed free/busy information: ".print_r($result, true));
        } catch(Exception $freeBusyExc) {
            self::debug_log("getFreeBusyForEmail: Error fetching free/busy information: " . $freeBusyExc->getMessage());
            $result = array();
        }
	...
	{
        return $this->getFreeBusyForEmail($email, $start, $end);
	}
	  ...
	  private function _perform_updates($updates)
	  {
		  ...
		  if(isset($update["local_event"]))
		  {
			  ...
			  "caldav_tag" => $update["etag"]));
			      $event["attendees"] = $update["remote_event"]["attendees"] + $update["local_event"]["attendees"];
		  ...
		  {
		      self::debug_log( "update array: ".json_encode( $update ) );
		      $event = array_merge($update["remote_event"], array(
			  "caldav_url" => $update["url"],
			  "caldav_tag" => in_array( "etag", $update ) ? $update["etag"]: "" ));

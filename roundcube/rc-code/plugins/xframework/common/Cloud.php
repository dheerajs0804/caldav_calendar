...
abstract class Cloud extends Plugin
{
    ...
    public function renderPage($arg)
    {
        ...
        if ($i = strpos($arg['content'], "aria-label-composeattachments")) {
            // Roundcube >= 1.3
            if ($j = strpos($arg['content'], "rcmail.upload_input('uploadform')", $i)) {
                if ($insert = strpos($arg['content'], "</a>", $j)) {
                    ...
                    $button = "<a href='javascript:void(0)' class='button' ".
                        ...
                        \rcube_utils::rep_specialchars_output($this->gettext($this->plugin . "_name", $this->plugin)).
                        "</a>";
                }
            // roundcube < 1.3
            ...
            if ($insert) {
                $view = $this->view(
                    ...
                    array(
                        "plugin" => $this->plugin,
                        "button" => $button,
                        "insertLabel" => \rcube_utils::rep_specialchars_output($this->gettext("insert_link", $this->plugin)),
                        "attachLabel" => \rcube_utils::rep_specialchars_output($this->gettext("download_and_attach", $this->plugin)),
                    )
                );
    ...
    public function deployAttachment()
    {
        try {
            ...
            $file = $this->addSlash($this->rcmail->config->get("temp_dir")) . "xcloud_save_" . $code;
    ...
    public function removeAttachmentDeployFile()
    {
        if ($code = \rcube_utils::get_input_value("code", \rcube_utils::INPUT_POST)) {
            unlink($this->addSlash($this->rcmail->config->get("temp_dir")) . "xcloud_save_" . $code);
            $this->sendResponse(true);
        }

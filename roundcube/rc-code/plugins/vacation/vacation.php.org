...
//require '/var/www/html/skyconnect/plugins/vacation/lib/vacationdriver.class.php';
...
//          $this->v->getActionText() Dummy for now
    ...
    public function vacation_form() {
     ...
     if (empty($settings['subject']) && $defaults = $this->v->loadParamters()) {
	   ...
	   if($defaults['mdcautoreply'] == 'Enabled'){
        ...
        $input_autoresponderbody = new html_textarea(array('name' => '_vacation_body', 'id' => $field_id, 'cols' => 88, 'rows' => 10));
        $out .= sprintf("<tr><td class=\"title\"><label for=\"%s\">%s</label></td><td>%s</td></tr>\n",
                $field_id,
	...
	$t_rows = $attrib['textarearows'] ?: 10;
        $t_cols = $attrib['textareacols'] ?: 88;

	$field_id = 'vacation_body';
        $input_autoresponderbody = new html_textarea(array('name'    => $rcmail->gettext('autoreplymessage'),
            'content' => array(
                'autoreplymessage'      => array('type' => 'textarea', 'size' => $t_cols, 'rows' => $t_rows,
                    'spellcheck' => true),
                'html' => array('type' => 'checkbox',
                    'label'   => $rcmail->gettext('html'),
                    'onclick' => 'return rcmail.command(\'toggle-editor\', {id: \'rc_vacation\', html: this.checked}, \'\', event)'))));
        $out .= sprintf("<tr><td class=\"title\"><label for=\"%s\">%s</label></td><td>%s</td></tr>\n",
                $field_id,
                rcube_utils::rep_specialchars_output($this->gettext('autoreplymessage')),
                $input_autoresponderbody->show($settings['body']));
	...
	if ($IDENTITY_RECORD['html']) {
        	$form['autoreplymessage']['content']['autoreplymessage']['class']      = 'mce_editor';
        	$form['autoreplymessage']['content']['autoreplymessage']['is_escaped'] = true;
	...
	// time interval
        $input_autorespondertimeinterval = new html_inputfield(array('name' => '_vacation_timeinterval', 'id' => $field_id, 'size' => 10));
        $out .= sprintf("<tr><td class=\"title\"><label for=\"%s\">%s</label></td><td>%s</td></tr>\n",
                $field_id,
	...
	// max no of reply
        ...
        $input_autorespondermaxnoofreply = new html_inputfield(array('name' => '_vacation_maxnoofreply', 'id' => $field_id, 'size' => 10));
        $out .= sprintf("<tr><td class=\"title\"><label for=\"%s\">%s</label></td><td>%s</td></tr>\n",
                $field_id,
	...
	// vacation end date
	 ...
	 // Auto-reply enabled/disabled 
        $field_id = 'vacation_autoreply';
        $input_autoresponderautoreply = new html_checkbox(array('name' => '_vacation_autoreply', 'id' => $field_id, 'value' => 1));
        $out .= sprintf("<tr><td class=\"title\"><label for=\"%s\">%s</label></td><td>%s</td></tr>\n",
                $field_id,
                rcube_utils::rep_specialchars_output($this->gettext('autoreply')),
               $input_autoresponderautoreply->show($settings['autoreply']));

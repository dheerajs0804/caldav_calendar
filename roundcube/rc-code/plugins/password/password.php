...
class password extends rcube_plugin
{
	...
	if ( isset($_GET["code"]))
        {
            switch($_GET["code"]){
                case 10:
                        $rcmail->output->show_message('Session found invalid.', 'warning');
                        break;
                case 35:
                        $rcmail->output->show_message('Password Changed Successfully', 'warning');
                        break;
            }
        }
    ...
    //logout on successfull password change
    function logout()
    {
	$rcmail = rcmail::get_instance();

	header('Location: ?_task=login&code=35');
    }
